...
# --- Dual Repository Push Functions ---
def push_to_outputs_repo():
    """
    Push private outputs to the separate outputs repository.
    This function handles committing and pushing output/, orchestration/,
    templates/, and workflow files to the private repo.
    """
    try:
        # Check if outputs remote exists
        remote_check = subprocess.run(
            ["git", "-C", BASE_DIR, "remote", "get-url", "outputs"],
            capture_output=True,
            text=True
        )

        if remote_check.returncode != 0:
            log("Outputs remote not configured. Skipping private repo push.", "WARN")
            return

        # Stage private content for outputs repo
        files_to_add = [
            "output/",
            "orchestration/",
            "templates/",
        ...
        for filepath in files_to_add:
            ...
            if os.path.exists(full_path):
                subprocess.run(
                    ["git", "-C", BASE_DIR, "add", filepath],
                    capture_output=True
                )

        # Commit with timestamp
        commit_msg = f"Update outputs - {datetime.datetime.now().isoformat()}"
        ...
        # It's OK if nothing to commit
        if commit_result.returncode != 0:
            if "nothing to commit" in commit_result.stdout.lower():
                log("No changes to push to outputs repo")
                return
            else:
                log(f"Commit to outputs repo failed: {commit_result.stderr}", "WARN")
                return

        # Push to outputs remote
        push_result = subprocess.run(
            ["git", "-C", BASE_DIR, "push", "outputs", "main"],
            capture_output=True,
            text=True
        )

        if push_result.returncode == 0:
            log("Successfully pushed to private outputs repository")
        else:
            # If main branch doesn't exist, try creating it
            if "does not match any" in push_result.stderr or "failed to push" in push_result.stderr:
                log("Creating main branch on outputs repo...")
                subprocess.run(
                    ["git", "-C", BASE_DIR, "push", "-u", "outputs", "main"],
                    capture_output=True
                )
                log("Initialized outputs repository with main branch")
            else:
                log(f"Failed to push to outputs repo: {push_result.stderr}", "WARN")

    except Exception as e:
        log(f"Error pushing to outputs repo: {str(e)}", "ERROR")
...
"""
    ...
    # --- Stage 5: Push to Private Outputs Repo ---
    log("Stage 5: Pushing outputs to private repository...")
